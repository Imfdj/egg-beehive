...
class _objectName_Service extends Service {
  async findAll(payload) {
    ...
    return await ctx.model.Permissions.findAndCountAll({
      ctx, limit, offset, where, order: Order,
    });
  }
  ...
  async update(payload) {
    ...
    const one = await ctx.model.Permissions.findOne({ where: { id: { [ Op.not ]: id }, url, action } });
    if (one) {
      const err = new Error('已存在');
      err.parent = {};
      err.parent.errno = 1062;
    ...
    return await ctx.model.Permissions.update(payload, { ctx, where: { id: payload.id } });
  }

  async destroy(payload) {
    const { ctx } = this;
    const delData = await ctx.model.Permissions.findAll({ where: { id: payload.ids } });
    return await ctx.model.Permissions.destroy({ ctx, where: { id: payload.ids }, delData });
  }
